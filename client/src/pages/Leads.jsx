  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Sales Pipeline & Leads</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Track and advance opportunities through the 6-stage sales funnel</p>
        </div>
          ...
          <div className="flex items-center bg-white dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
            <button
              ...
              className={`flex items-center px-3 py-1.5 text-xs font-semibold rounded-lg transition ${
                viewMode === 'kanban' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center px-3 py-1.5 text-xs font-semibold rounded-lg transition ${
                viewMode === 'table' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="text-center sm:text-left border-r border-slate-100 dark:border-slate-800 last:border-none px-2">
        ...
        <div className="text-center sm:text-left px-2">
          ...
          <span className="text-xl font-bold text-slate-800 dark:text-slate-100 mt-1 block">{leads.length} Leads</span>
        </div>
      </div>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2 text-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400 rounded-xl focus:outline-none focus:border-blue-500 shadow-xs"
        />
      </div>
      ...
      {viewMode === 'kanban' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 overflow-x-auto pb-6">
          {PIPELINE_STAGES.map((stage) => {
            ...
            return (
              <div key={stage.id} className="flex flex-col bg-slate-100/70 dark:bg-slate-900/60 border border-slate-200/80 dark:border-slate-800 rounded-2xl p-3 min-w-[240px] max-h-[78vh]">
                <div className="flex items-center justify-between pb-2 mb-3 border-b border-slate-200 dark:border-slate-800">
                  <div>
                    <h3 className="text-xs font-bold text-slate-800 dark:text-slate-100 uppercase tracking-wider">{stage.label}</h3>
                    <span className="text-[11px] font-medium text-slate-500 dark:text-slate-400">${stageTotal.toLocaleString()}</span>
                  </div>
                  <span className="px-2 py-0.5 text-xs font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full border border-slate-200 dark:border-slate-700 shadow-xs">
                    {stageLeads.length}
                  </span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-1">
                  {stageLeads.length === 0 ? (
                    ...
                    stageLeads.map((lead) => (
                      <div key={lead._id} className="bg-white dark:bg-slate-800/90 p-3.5 rounded-xl border border-slate-200/90 dark:border-slate-700 shadow-xs hover:shadow-md transition space-y-2">
                        <div className="flex items-start justify-between">
                          <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">{lead.source}</span>
                          <span className="font-bold text-xs text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-700/60 px-2 py-0.5 rounded-md">
                            ${(lead.value || 0).toLocaleString()}
                          </span>
                        </div>
                        <h4 onClick={() => handleOpenDetail(lead)} className="font-bold text-sm text-slate-800 dark:text-white hover:text-blue-500 cursor-pointer line-clamp-2">
                          {lead.title}
                        </h4>
                        <div className="text-xs text-slate-500 dark:text-slate-400 space-y-0.5">
                          <div className="font-medium text-slate-700 dark:text-slate-300 truncate flex items-center">
                        ...
                        <div className="pt-2 border-t border-slate-100 dark:border-slate-700/80 flex items-center justify-between text-xs">
                          <select
                            ...
                            className="text-[11px] bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg px-2 py-1 text-slate-700 dark:text-slate-200 font-medium"
                          >
                          ...
                          <div className="flex items-center space-x-1">
                            {!lead.isConverted && (
                              <button
                                ...
                                className="p-1 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/40 rounded-md transition"
                              >
                        ...
                        {lead.isConverted && (
                          <div className="inline-flex items-center text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded-full border border-emerald-200 dark:border-emerald-800">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {leads.map((lead) => (
                  <tr key={lead._id} className="hover:bg-slate-50/80 dark:hover:bg-slate-850 transition">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-slate-900 dark:text-white cursor-pointer hover:text-blue-500" onClick={() => handleOpenDetail(lead)}>
                        {lead.title}
                      </div>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400">{lead.contactName} ({lead.contactEmail})</div>
                    </td>
                    <td className="px-6 py-4">
                      <select
                        value={lead.status}
                        onChange={(e) => handleUpdateStatus(lead._id, e.target.value)}
                        className="text-xs font-semibold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 text-slate-700 dark:text-slate-200"
                      >
                        {PIPELINE_STAGES.map((s) => (
                          <option key={s.id} value={s.id}>{s.label}</option>
                        ))}
                    ...
                    <td className="px-6 py-4 font-bold text-slate-900 dark:text-white">${(lead.value || 0).toLocaleString()}</td>
                    <td className="px-6 py-4 text-xs text-slate-600 dark:text-slate-300">{lead.assignedTo?.name || 'Unassigned'}</td>
                    <td className="px-6 py-4 text-right space-x-1">
                      {!lead.isConverted && (
                        <button
                          onClick={() => handleConvertLead(lead._id)}
                          className="px-2.5 py-1 text-xs font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-900 transition"
                        >
                          Convert
                        </button>
                      )}
                      <button onClick={() => handleOpenDetail(lead)} className="p-1.5 text-slate-500 hover:text-blue-500 dark:hover:bg-slate-800 rounded-lg transition">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleDeleteLead(lead._id)} className="p-1.5 text-slate-500 hover:text-rose-500 dark:hover:bg-slate-800 rounded-lg transition">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-lg w-full p-6 overflow-hidden">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-lg font-bold text-slate-800 dark:text-white">Create New Sales Lead</h2>
            ...
            <form onSubmit={handleCreateLead} className="mt-4 space-y-3 text-xs">
              ...
              <div className="flex justify-end space-x-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                <button type="button" onClick={() => setShowAddModal(false)} className="px-4 py-2 font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition">
                  Cancel
                </button>
                <button type="submit" className="px-4 py-2 font-semibold text-white bg-blue-600 hover:bg-blue-500 rounded-xl shadow-md shadow-blue-500/20 transition">
                  Save Lead
                </button>
              </div>
            </form>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl max-w-2xl w-full p-6 max-h-[90vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">{selectedLead.company || selectedLead.contactName} • ${(selectedLead.value || 0).toLocaleString()} • Stage: {selectedLead.status}</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto py-4 space-y-4">
              {!selectedLead.isConverted ? (
                <div className="p-3.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 rounded-xl flex items-center justify-between">
                  <span className="text-xs text-emerald-800 dark:text-emerald-200 font-medium">Ready to close this deal? Convert to a Customer record.</span>
                  <button
                    onClick={() => {
                      handleConvertLead(selectedLead._id);
                      setShowDetailModal(false);
                    }}
                    className="px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-lg shadow-sm transition"
                  >
                    Convert to Customer
                  </button>
                </div>
              ) : (
                <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl text-xs text-slate-600 dark:text-slate-300 flex items-center">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 mr-2" /> This deal was converted into a Customer account.
                </div>
              )}

              <div className="p-4 border border-blue-100 dark:border-blue-900/40 bg-blue-50/30 dark:bg-blue-950/30 rounded-xl">
                ...
                <form onSubmit={handleAddNote} className="space-y-2">
                    ...
                    {['Note', 'Call', 'Meeting', 'Email'].map((t) => (
                      <button
                        ...
                        className={`px-2 py-1 text-[11px] font-semibold rounded-lg ${
                          activityType === t ? 'bg-blue-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700'
                        }`}
                      >
                        {t}
                      </button>
                  ...
                  <div className="flex space-x-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 text-xs bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-100 rounded-xl focus:outline-none"
                    />
                    <button type="submit" className="px-3 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl flex items-center shadow-xs">
              ...
              <div>
                ...
                {activities.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {activities.map((act) => (
                      <div key={act._id} className="p-3 bg-white dark:bg-slate-800/80 border border-slate-100 dark:border-slate-700/60 rounded-xl text-xs flex items-start space-x-2">
                        <div className="w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900/60 text-blue-600 dark:text-blue-300 flex items-center justify-center font-bold text-[10px] shrink-0">
                          {act.type.charAt(0)}
                        </div>
            ...
            <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-end">
              <button onClick={() => setShowDetailModal(false)} className="px-4 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-semibold rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition">
                Close
              </button>
            </div>
          </div>
